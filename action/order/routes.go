...
func Router() chi.Router {
	...
	r.Route("/{order_id}", func(r chi.Router) {
		r.Get("/", details)   // GET /orders/{id} - read a single order by :order_id
		r.Put("/", update)    // PUT /orders/{id} - update a single order by :order_id
		r.Delete("/", delete) // DELETE /orders/{id} - delete a single order by :order_id
